...
def add_product(category_id, name, scientific_name, description, price, stock, image,
    ...
    with get_cursor(dictionary=False, commit=True) as cur:
        cur.execute("""
            ...
            (category_id, product_name, scientific_name, short_description,
             description, price, stock, image, habitat, life_cycle,
             care_instructions, fun_facts, badge)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """, (
            category_id, name, scientific_name, short_description,
            description, price, stock, image, habitat,
            life_cycle, care_instructions, fun_facts, badge
        ))
...
def get_wishlist_product_ids(user_email):
    with get_cursor() as cur:
        cur.execute(
            "SELECT product_id FROM wishlist WHERE user_email=%s",
            (user_email,)
        )
